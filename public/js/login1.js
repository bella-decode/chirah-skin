...
spans.map((span) => {
    span.addEventListener('click', (e)=> {
        ...
        document.querySelector(":root").style.setProperty("--main-color1", color); 
...
Array.from(inputs).map(input=> {
    icons.map(icon => {
        icon.innerHTML = '<img src="/public/img/eye.svg" alt="">';

        icon.addEventListener('click', ()=> {
          const type = input.getAttribute("type");
          if(type === 'password'){
            input.setAttribute("type", "text");
            icon.innerHTML = '<img src="/public/img/hide.svg" alt="">'
          }else if(type === 'text'){
